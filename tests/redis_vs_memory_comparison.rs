...
// TODO: NOT PART OF M1 - All tests in this file use old EventFluxQL syntax
    ...
    if let Some(last_event) = out.last() {
        if let Some(AttributeValue::Long(count)) = last_event.get(1) {
            ...
            if *count == 4 {
                ...
                println!(
                    "❌ Memory store with count fails: got count={}, expected count=4",
                    count
                );
            }
        } else {
    ...
    if let Some(last_event) = out.last() {
        if let Some(AttributeValue::Long(count)) = last_event.get(1) {
            ...
            if *count == 4 {
                ...
                println!(
                    "❌ Redis store failed: got count={}, expected count=4",
                    count
                );
            }
        } else {
